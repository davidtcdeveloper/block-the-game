...
data class GameState(
    ...
    fun clearLines(): GameState {
        val linesToClearIndices = board.mapIndexedNotNull { index, row ->
        ...
        linesToClearIndices.sortedDescending().forEach { lineIndex -> // Iterate descending to avoid index issues
            newBoard.removeAt(lineIndex)
            newBoard.add(0, List(boardWidth) { false })
        }

        val linesClearedCount = linesToClearIndices.size
        val pointsEarned = when (linesClearedCount) {
            1 -> 100
            2 -> 300
            3 -> 500
            4 -> 800
